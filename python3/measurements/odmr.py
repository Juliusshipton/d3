...
class ODMR(ManagedJob, GetSetItemsMixin):
    ...
    traits_view = View(VGroup(HGroup(Item('submit_button', show_label=False, springy=True),
                              ...
                              VGroup(HGroup(Item('power', width= -40, enabled_when='state != "run"', springy=True),
                                            Item('frequency_begin', width= -80, enabled_when='state != "run"', springy=True),
                                            Item('frequency_end', width= -80, enabled_when='state != "run"', springy=True),
                                            Item('frequency_delta', width= -80, enabled_when='state != "run"', springy=True),
                                            ),
                                     HGroup(Item('pulsed', enabled_when='state != "run"', springy=True),
                                            Item('power_p', width= -40, enabled_when='state != "run"', springy=True),
                                            Item('frequency_begin_p', width= -80, enabled_when='state != "run"', springy=True),
                                            Item('frequency_end_p', width= -80, enabled_when='state != "run"', springy=True),
                                            Item('frequency_delta_p', width= -80, enabled_when='state != "run"', springy=True),
                                            Item('t_pi', width= -50, enabled_when='state != "run"', springy=True),
                                            ),
                                     HGroup(Item('seconds_per_point', width= -40, enabled_when='state != "run"', springy=True),
                                            Item('laser', width= -50, enabled_when='state != "run"', springy=True),
                                            Item('wait', width= -50, enabled_when='state != "run"', springy=True),
                                            ),
                                     HGroup(Item('perform_fit', springy=True),
                                            Item('number_of_resonances', width= -60, springy=True),
                                            Item('threshold', width= -60, springy=True),
                                            Item('n_lines', width= -60, springy=True),
                                            ),
